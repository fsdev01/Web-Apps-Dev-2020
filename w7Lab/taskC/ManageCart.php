...
<?php
    ...
    if ($_SESSION["Cart"] != "") {
        ...
        if ($action == "Add"){
            ...
            if (isset($cart[$title]) && $cart[$title] != ""){  
...
function toXml($MDA){
    ...
    if($MDA!=""){
        ...
        foreach ($MDA as $Item => $ItemDetails)
        {
            ...
            $idElement = $book->appendChild($idElement);
            $value5 = $doc->createTextNode($ItemDetails['id']);
            $value5 = $idElement->appendChild($value5);          
