  ...
  if (xHRObject.readyState == 4 && xHRObject.status == 200) {
    ...
    if (header != null) {
      ...
      for (var i = 0; i < header.length; i++) {
        ...
        rowString +=
          "<td><a href='#' onclick='AddRemoveItem(\"Remove\"," +i +  ");'>Remove Item</a></td>";
...
function AddRemoveItem(action, bookNo) {
  ...
  var book = document.getElementsByClassName("book")[bookNo].innerHTML;
  // Parameter 2: Get the ISBN of the book
  var isbn = document.getElementsByClassName("ISBN")[bookNo].innerHTML;
  // Parameter 3: Get the price of the book
  var price = document.getElementsByClassName("price")[bookNo].innerHTML;
  // Paramter 4: Book Id
  var id = bookNo;
